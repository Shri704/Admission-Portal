...
import { getStudentFees } from "../../services/feeService.js";
...
export default function FeePayment() {
    ...
    async function loadFees() {
      try {
        ...
        const message =
          ...
          "Unable to fetch fee details for the selected year.";
  ...
  const handlePay = async () => {
    ...
    if (!feeData.total) {
      showToast("No payable fees found for the selected year.", "info");
      return;
    }

    if (feeData.pending <= 0) {
      showToast("You have already cleared the fees for this year.", "success");
      return;
    }
    ...
    try {
      ...
      const response = await initiatePayment({ amount, year: yearNumber, feeType: "Academic" });
      ...
      const options = {
        ...
        description: `Fee payment for ${selectedYearLabel || `Year ${selectedYear}`}`,
        order_id: order.id,
    ...
    <div className="space-y-10 rounded-2xl bg-gradient-to-br from-white via-indigo-50/40 to-blue-50/20 p-6 sm:p-8 md:p-10 shadow-[0_10px_40px_rgba(15,23,42,0.05)]">
      ...
      <header className="border-b border-indigo-100/50 pb-5">
        ...
        <p className="mt-1 max-w-2xl text-sm text-gray-600">
          Select your academic year to view the applicable fee structure and pay securely
          through Razorpay.
        </p>
      </header>
